...
func (q ReadReq) MarshalBinary() ([]byte, error) {
	...
	// operation code + filename + 0 byte + mode + 0 byte (the request packet structure)
	cap := 2 + 2 + len(q.Filename) + 1 + len(q.Mode) + 1

	b := new(bytes.Buffer)
	b.Grow(cap)

	err := binary.Write(b, binary.BigEndian, OpRRQ) // write operation
	if err != nil {
		return nil, err
	}
...
func (e *Err) UnmarshalBinary(p []byte) error {
	...
	err := binary.Read(r, binary.BigEndian, &code) // read the operation code
	if err != nil {
		return err
	}
...
type Server struct {
	...
	Retries uint8         // the number of times to rety a failed transmission
